...
########################################################
# Script for deleting n subscribors
#
# Usage:
#   ./delete-subscribors.sh <n>
#
# Description:
#   This script is used to delete n subscribors.
########################################################
...
main() {
    ...
    for i in $(seq 1 $n); do
        ...
        if curl -s --fail -X DELETE $FREE5GC_CONSOLE_BASE_URL/api/subscriber/imsi-$imsi/$plmn_id -H "Token: $token"; then
            echo "Subscriber imsi-$imsi deleted successfully!"
        else
            echo "Failed to delete subscriber imsi-$imsi!"
        fi
